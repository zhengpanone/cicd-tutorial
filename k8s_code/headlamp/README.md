...
| 资源类型 | 名称 | 说明 |
|---|---|---|
| Namespace | headlamp | 独立部署命名空间，便于隔离和清理 |
| ServiceAccount | headlamp-admin | Headlamp 管理员身份 |
...
1. **Kubernetes 上下文**：确认当前上下文为 docker-desktop
...
# 检查状态
kubectl get pods -n headlamp -l app=headlamp
```
...
```bash
# 查看日志
kubectl logs -n headlamp deployment/headlamp --tail=50

# 重启
kubectl rollout restart deployment/headlamp -n headlamp

# 进入容器
kubectl exec -it $(kubectl get pod -n headlamp -l app=headlamp -o name) -n headlamp -- sh
```
...
Headlamp 官方文档推荐用 ServiceAccount Token 登录。本清单创建了独立命名空间中的 `headlamp/headlamp-admin` 并绑定 `cluster-admin`，请优先使用下面的 Token：
...
如果 `/me` 返回的 `username` 是 `system:serviceaccount:headlamp:headlamp-admin`，说明 Headlamp、Token 和 RBAC 都是正常的。如果页面仍然失败，在浏览器中清理当前访问地址的站点数据后重新登录：
